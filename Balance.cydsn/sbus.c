...
/* update the channels from the last received packet */
...
/* get value normalized to [-100,100] */
...
/* get channel value between -128 and 127 */
signed char sbus_getByteChannel(int channel) {
    if (channel < 1 || channel > 18) {
        return 0;
    } else {
        int result = lroundf((_channels[channel - 1] - 172) / 6.4528) - 127;
        if (result > 127) {
            result = 127;
        } else if (result < -128) {
            result = -128;
...
/* failsafe is activated when connection is lost */
